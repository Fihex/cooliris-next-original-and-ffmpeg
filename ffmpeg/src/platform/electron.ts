...
export interface FfProbe {
  ...
  audios: { index: number; codec: string; lang?: string; title?: string }[];
  subtitles: { index: number; codec: string; lang?: string; title?: string; text: boolean }[];
}

/** Result of preparing a non-native video: a real, seekable temp .mp4 served over
 *  coolmedia://, plus its duration, audio tracks, and embedded subtitles (WebVTT). */
export interface FfPrepare {
  videoUrl: string;
  durationSec: number;
  audios: { index: number; label: string }[];
  subs: { label: string; vtt: string }[];
}

// Containers Chromium can demux on its own; others are prepared (remux/transcode to a
// temp .mp4) on open. The marker URL just carries the absolute path to the player.
