...
//
// Generates the libVLC plugin cache (plugins.dat) inside the *packed* vendor\plugins so
// libVLC loads its module bank from the cache instead of rescanning every plugin DLL on
// each launch — that scan is ~30s on Windows (the install dir is read-only, so libVLC
// can't persist a cache it builds at runtime). VLC validates the cache by each plugin's
// mtime+size, so it must be built against the final packed files; afterPack runs after
// the app dir is assembled and before the nsis/dir targets are produced from it, so both
// the installer and the unpacked folder ship a valid cache with matching timestamps.
